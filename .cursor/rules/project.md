...
## PROTECTED FILES — NEVER READ OR WRITE

These files are off-limits under any circumstances. Do not read, edit, create, or suggest changes to them:

- `.env`, `.env.*`, `.env.local`, `.env.production`, `.env.staging`
- `**/*.pem`, `**/*.key`, `**/secrets/**`
- `stripe_keys.txt`, any file containing `STRIPE_*` secrets
- `**/db/migrations/**` — never edit existing migrations; create new ones only

If a task requires touching any of the above, stop and ask the user to handle it manually.
