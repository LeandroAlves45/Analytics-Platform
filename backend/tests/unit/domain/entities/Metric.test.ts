...
describe("Metric Entity", () => {
  ...
  describe("constructor - validation errors", () => {
    it("should throw ValidationError when workspaceId is empty", () => {
      expect(() => new Metric({ ...validInput, workspaceId: "" })).toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError when apiKeyId is empty", () => {
      expect(() => new Metric({ ...validInput, apiKeyId: "" })).toThrow(
        ValidationError,
      );
    });
    ...
    it("should throw ValidationError when endpoint does not start with /", () => {
      expect(
        () => new Metric({ ...validInput, endpoint: "api/users" }),
      ).toThrow(ValidationError);
    });

    it("should throw ValidationError when method is invalid", () => {
      expect(() => new Metric({ ...validInput, method: "INVALID" })).toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError when latencyMs is zero or negative", () => {
      expect(() => new Metric({ ...validInput, latencyMs: -100 })).toThrow(
        ValidationError,
      );
      expect(() => new Metric({ ...validInput, latencyMs: 0 })).toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError when statusCode is below 100 or above 599", () => {
      expect(() => new Metric({ ...validInput, statusCode: 99 })).toThrow(
        ValidationError,
      );
      expect(() => new Metric({ ...validInput, statusCode: 600 })).toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError when requestId is empty", () => {
      expect(() => new Metric({ ...validInput, requestId: "" })).toThrow(
        ValidationError,
      );
    });

    it("should throw ValidationError when payloadSizeBytes is negative", () => {
      expect(() => new Metric({ ...validInput, payloadSizeBytes: -1 })).toThrow(
        ValidationError,
      );
    });
