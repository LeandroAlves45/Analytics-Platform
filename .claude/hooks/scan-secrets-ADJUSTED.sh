...
# ADJUSTED FOR PT MANAGER: Added Stripe key patterns
...
if echo "$CONTENT" | grep -qE '(sk_test_|pk_test_|rk_test_)[a-zA-Z0-9]{20,}'; then
  MATCHES="$MATCHES Stripe test key;"
fi

# Stripe live keys (sk_live_..., pk_live_...) - CRITICAL
if echo "$CONTENT" | grep -qE '(sk_live_|pk_live_|rk_live_)[a-zA-Z0-9]{20,}'; then
  MATCHES="$MATCHES Stripe LIVE key (CRITICAL DANGER!);"
fi
# =================================================
